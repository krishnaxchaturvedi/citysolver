...
import { computeRecommendation, type Recommendation } from "@/lib/map-data"
...
export function RecommendationPanel({
  ...
  const recommendation = React.useMemo<Recommendation | null>(() => {
    ...
    return computeRecommendation(category, lat, lng)
  }, [category, lat, lng])
    ...
    <div className="space-y-3">
      <AnimatedCard>
        <div className="relative overflow-hidden rounded-xl border border-primary/20 bg-gradient-to-br from-primary/10 via-primary/5 to-transparent p-4">
          ...
          <div className="relative flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold">Smart Recommendation Engine</h3>
      ...
      <RecoRow icon={IndianRupee} iconBg="bg-warning/10" iconColor="text-warning-foreground" label="Estimated Budget" delay={240}>
        <p className="text-lg font-bold text-warning-foreground">
          <AnimatedCounter
            value={recommendation.estimatedBudget.amount}
            prefix="₹"
            duration={1800}
          />
        </p>
      ...
      <RecoRow icon={Calendar} iconBg="bg-success/10" iconColor="text-success" label="Expected Completion Date" delay={400}>
        <p className="text-sm font-semibold">{recommendation.expectedCompletionDate}</p>
      </RecoRow>

      <RecoRow icon={Lightbulb} iconBg="bg-yellow-500/10" iconColor="text-yellow-600" label="Alternative Solution" delay={480}>
        <p className="text-xs text-muted-foreground">{recommendation.alternativeSolution}</p>
      </RecoRow>
          ...
          <CardContent>
            <div className="space-y-2">
              {recommendation.preventiveMeasures.map((measure, i) => (
                <div
                  key={i}
                  className="flex items-start gap-2 rounded-lg bg-muted/30 p-2.5 text-xs text-muted-foreground animate-in fade-in slide-in-from-left-2 duration-500"
      ...
      {recommendation.nearbySimilarCases.length > 0 && (
        <AnimatedCard delay={640}>
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center gap-2 text-sm">
                <MapPin className="size-4 text-primary" aria-hidden="true" />
                Nearby Similar Cases
              </CardTitle>
              <CardDescription className="text-xs">Similar complaints within 800m radius</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {recommendation.nearbySimilarCases.map((c, i) => (
                  <a
                    key={c.id}
                    href={`/tracking/${c.id}`}
                    className="flex items-center justify-between rounded-lg border border-border p-2.5 transition-colors hover:bg-accent focus-visible:outline-2 focus-visible:outline-ring focus-visible:outline-offset-2"
                  >
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-mono font-semibold">{c.id}</p>
                      <p className="text-xs text-muted-foreground truncate">{c.title}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <Badge variant="outline" className="text-xs">{c.distance}</Badge>
                      <Badge variant="outline" className="text-xs bg-muted/50">{c.status}</Badge>
