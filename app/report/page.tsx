...
export default function ReportPage() {
  ...
  const handleCategorySelect = React.useCallback((key: CategoryKey) => {
    setCategory(key)
  }, [])
  ...
  const handleSubmit = React.useCallback((e: React.FormEvent) => {
    e.preventDefault()
    if (!isValid) {
      toast.error("Please complete all fields", { description: "Category, description (min 20 chars), location, and photo are required." })
      return
    }
    setSubmitting(true)
    setTimeout(() => {
  ...
  const detectLocation = React.useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        setCoords(c)
        setLocating(false)
        toast.success("Location detected", { description: `Coordinates: ${c}` })
      },
      () => {
        setLocating(false)
        toast.error("Location access denied", { description: "Please enable location access or enter address manually." })
      },
